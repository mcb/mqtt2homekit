...
class Accessory(accessory.Accessory):
	def __init__(self, *args, **kwargs):
		...
		self._model = f'MQTT Bridged {services[0]}'
	...
	def get_services(self, name):
		return [service for service in self.services if service.display_name == name]
	...
	def no_response(self):
		LOGGER.debug(f'Marking {self} as Not Responding')
