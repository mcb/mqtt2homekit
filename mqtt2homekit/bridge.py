...
def build_driver(bridge, port, persist_file):
	driver = AccessoryDriver(
		port=port, persist_file=persist_file, encoder=BridgeEncoder(bridge)
	)
...
class MQTTBridge(Bridge):
	...
	def add_info_service(self):
		...
		info_service.configure_char(
			'SerialNumber', value='02b26429-cb8b-47cd-a27a-2feb33996cde'
		)
	...
	def add_accessory(self, accessory):
		...
		def add_characteristic(service, characteristic):
			LOGGER.debug(
				f'Set setter_callback for {accessory}: {service}.{characteristic}'
			)
			characteristic.setter_callback = partial(
				self.send_mqtt_message, accessory, service, characteristic
			)
	...
	def on_connect(self, client, userdata, flags, reason_code, properties):
		client.subscribe(f'{self.prefix}/#', 1)
	...
	async def run(self):
		...
		self.client.message_callback_add(
			f'{self.prefix}/+/+/+', self.handle_mqtt_message
		)
		self.client.message_callback_add(
			f'{self.prefix}/+/+/+/+', self.handle_mqtt_message
		)
		try:
			self.client.connect(
				self.mqtt_server.hostname,
				port=self.mqtt_server.port or 1883,
				keepalive=30,
			)
	...
	def flag_unseen(self):
		...
		for acc in list(self.accessories.values()):
			if acc._last_seen and now - acc._last_seen > ONE_HOUR:
				...
				LOGGER.info(
					f'Look like {acc.accessory_id} has no data for {interval} seconds'
				)
	...
	def handle_mqtt_message(self, client, userdata, message):
		try:
			...
			_prefix, accessory_id, service_type, index, characteristic = (
				message.topic.split('/')
			)
			index = int(index)
		...
		if not message.payload:
			...
			LOGGER.info(f'REMOVE {accessory_id}: {service_type}.{characteristic}')
		...
		try:
			...
			LOGGER.debug(
				f'SET {accessory_id}: {service_type}[{index}].{characteristic} -> {value}'
			)
			...
			LOGGER.error(
				f'Exception handling message {exc.__class__.__name__}: {exc.args}'
			)
	...
	def send_mqtt_message(self, accessory, service, characteristic, value):
		...
		if service == 'AccessoryInformation':
			LOGGER.info(
				f'Received AccessoryInformation message: {accessory}: {service}.{characteristic} -> {value}'
			)
			return

		if characteristic.display_name == 'Identify':
			LOGGER.info(f'Identify: {accessory}')
			return

		service_topic_name = service.display_name
		if len(accessory.get_services(service_topic_name)) > 1:
			service_topic_name += f'/{accessory.get_service_index(service)}'

		try:
			characteristic.set_value(value)
			topic = f'{self.prefix}/{accessory.accessory_id}/{service_topic_name}/{characteristic.display_name}'
			LOGGER.debug(topic)
			...
			LOGGER.error(f'Exception sending message: {exc.args}')
