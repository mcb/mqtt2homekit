...
def test_handle_mqtt_messages(bridge, mocker):
	...
	bridge.handle_mqtt_message(
		None, None, Message(topic=b'__TEST__/Foo/Lightbulb/On', payload=b'1')
	)
	Accessory.set_characteristic.assert_called_once()

	bridge.handle_mqtt_message(
		None, None, MQTTMessage(topic=b'__TEST__/Foo/AccessoryInformation/Name')
	)

	bridge.handle_mqtt_message(
		None, None, MQTTMessage(topic=b'__TEST__/Foo/Lightbulb/On')
	)
...
def test_accessory_with_multiple_services(bridge, mocker):
	mocker.patch('mqtt2homekit.accessory.Accessory.set_characteristic')
	bridge.handle_mqtt_message(
		None, None, Message(topic=b'__TEST__/Foo/Lightbulb/3/On', payload=b'1')
	)
	Accessory.set_characteristic.assert_called_once()
...
def test_setting_characteristics(bridge, mocker):
	...
	bridge.client.publish.assert_called_once_with(
		'__TEST__/Foo/Lightbulb/On', b'1', qos=2, retain=True
	)
	...
	bridge.client.publish.assert_called_with(
		'__TEST__/Foo/Lightbulb/Brightness', b'0', qos=2, retain=True
	)
	brightness.client_update_value(82)
	bridge.client.publish.assert_called_with(
		'__TEST__/Foo/Lightbulb/Brightness', b'82', qos=2, retain=True
	)
...
def test_setting_characteristics_multiple_services(bridge, mocker):
	...
	on.client_update_value(1)
	bridge.client.publish.assert_called_once_with(
		'__TEST__/Foo/Lightbulb/0/On', b'1', qos=2, retain=True
	)
